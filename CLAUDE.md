...
```bash
# Pull latest image and restart (after CI/CD builds new image)
ssh root@<SERVER_IP> 'cd /opt/purl && docker compose pull purl && docker compose up -d purl'
```

### Docker

- **Image**: `ismoilovdev/purl:latest` (prod), `ismoilovdev/purl:dev` (dev)
