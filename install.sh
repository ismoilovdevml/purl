...
# Vector hostname for log attribution
VECTOR_HOSTNAME=$server_hostname
EOF
...
install_agent() {
    ...
    if command -v journalctl &> /dev/null; then
        has_journald=true
        log_info "journalctl found - enabling journald source"
    else
        log_info "journalctl not found - using docker_logs only"
    fi
...
EOF
    ...
    # Add journald source only if available
...
EOF
    ...
    # Determine transform inputs based on available sources
    local transform_inputs="docker_logs"
    if [ "$has_journald" = true ]; then
        transform_inputs="docker_logs, journald"
    fi
...
type = "filter"
inputs = ["parsed"]
# Filter out empty messages and Vector internal logs
condition = '''
length(string!(.message)) > 0 &&
!starts_with(string!(.message), "vector::") &&
!contains(string!(.message), "vector::internal_events")
'''
