...
# Free plan defaults (no license required). Sourced from the canonical plan
# catalogue — NEVER inline a feature/limit literal here again: purl-web's
# stripe/config.ts and Purl::License::Plans must stay identical.
my $FREE_PLAN = Purl::License::Plans::free_plan();

# Trial plan = 14 days of Pro, taken from the SAME catalogue. Deriving it
# rather than listing it is the point: a hand-written trial list drifts, and
# every feature that is in trial but not in Pro vanishes the moment the
# customer pays.
my $TRIAL_FEATURES = Purl::License::Plans::features_for('pro');
my $TRIAL_LIMITS   = Purl::License::Plans::unlimited_limits();
my $TRIAL_DAYS     = 14;
...
# NOTE: quota enforcement lives in ONE place — Purl::API::Controller::Base's
# check_limit(), which every controller calls with the request's license
# stash. The middleware deliberately exposes no second limit checker:
#   * check_server_limit() was removed — it duplicated check_limit's `-1 means
#     unlimited` rule and had zero callers in lib/.
#   * get_retention_limit() was removed — nothing read it. Retention is set by
#     PURL_RETENTION_DAYS on the customer's own ClickHouse, so no plan can
#     enforce a retention number and no plan claims one.
...
plan type, feature list, quota limits, and expiry.

Plan entitlements are NOT defined here — they come from
L<Purl::License::Plans>, which is kept identical to purl-web's
C<src/lib/stripe/config.ts>. The free plan grants every self-hosted feature
with unlimited quotas (-1); the 14-day trial is exactly the Pro feature list.
